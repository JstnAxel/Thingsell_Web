...
function Show({product, variationOptions}: {product: Product, variationOptions: number[]}) {
  ...
  const chooseOption = (
    ...
    setSelectedOptions((prevSelectedOptions) =>{
      ...
      if (updateRouter) {
        router.get(url, {
          option: getOptionIdsMap(newOptions)
        }, {
          preserveScroll: true,
          preserveState: true
        })
            ...
            <div className="flex gap-2 mb-4">
              {type.options.map(option => (
                <div onClick={() => chooseOption(type.id, option)} key={option.id}>
                  {option.images && <img src={option.images[0].thumb} alt="" className={`w-[50px] ${selectedOptions[type.id]?.id === option.id ? 'outline outline-4 outline-primary' : ''}`}
                  />}
                </div>
              ))}
            </div>}
